...
class Solution:
    def minDiffInBST(self, root):
        ...
        if len(self.visit)==1:
            ...
            for i in range(1,len(self.visit)):
              self.diff.append(self.visit[i]-self.visit[i-1]) 
...
#               temp[i]+=[s]
#            else:
#               temp[i]+=[s.lower(),s.upper()]
#        return [ ''.join(x) for x in product(*temp)]
...
class Solution:
    def numberOfLines(self, widths, S):
        ...
        for s in S:
            ...
            if curr+widths[ord(s)-ord('a')]<=100:
               curr+=widths[ord(s)-ord('a')]
            else:
                 curr=0
                 line+=1
                 curr+=widths[ord(s)-ord('a')]
...
class Solution:
    def largestTriangleArea(self, points):
        ...
        for (x1,y1),(x2,y2),(x3,y3) in itertools.combinations( points,3):
            area=max(area,abs(x2*y3+x3*y1+x1*y2-x3*y2-y3*x1-x2*y1))
        return area/2
